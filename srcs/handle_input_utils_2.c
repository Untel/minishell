...
/*   Updated: 2021/05/12 22:58:47 by riblanc          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_ctrl_darrow(t_line *line, int edit, int n)
{
	if (n == 0)
		n = 1;
	if (line->pos + (g_sh.term.width * n) < line->lst_input->size + !edit)
		line->pos += g_sh.term.width * n;
	else
...
void	handle_ctrl_uarrow(t_line *line, int n)
{
	if (n == 0)
		n = 1;
	if (line->pos - (g_sh.term.width * n) > 0)
		line->pos -= (g_sh.term.width * n);
	else
		line->pos = 1;
}
